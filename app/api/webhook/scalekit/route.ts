    ...
    try {
        ...
        switch(event.type){
            case "user.organization_membership_created":
                ...
                await db.update(teamMembers).set({status:"active"}).where(eq(teamMembers.user_email,param.user.email))
                break;
            default:
        ...
        return NextResponse.json({success:true},{status:200})
         ...
         console.log("Webhook processing error",error)
          return NextResponse.json({error:"webhook error process failed"},{status:500})
